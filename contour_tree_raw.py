...
file_path='/home/sushmitha/Desktop/Adhitya/Baby.256x256x98.raw'
file_name = os.path.splitext(os.path.basename(file_path))[0]
dimensions=file_name.split(".")
[dim_x,dim_y,dim_z]=dimensions[1].split("x")
...
rawFile = ImageReader(FilePrefix=file_path)

# Properties modified on rawFile
rawFile.DataScalarType = 'unsigned char'
rawFile.DataExtent = [0, int(dim_x)-1, 0, int(dim_y)-1, 0, int(dim_z)-1]
...
# show data in view
rawFileDisplay = Show(rawFile, renderView1)
# trace defaults for the display properties.
rawFileDisplay.Representation = 'Outline'
rawFileDisplay.ColorArrayName = ['POINTS', '']
rawFileDisplay.OSPRayScaleArray = 'scalars'
rawFileDisplay.OSPRayScaleFunction = 'PiecewiseFunction'
rawFileDisplay.SelectOrientationVectors = 'None'
rawFileDisplay.ScaleFactor = 10.0
rawFileDisplay.SelectScaleArray = 'scalars'
rawFileDisplay.GlyphType = 'Arrow'
rawFileDisplay.ScalarOpacityUnitDistance = 1.7320508075688776
rawFileDisplay.Slice = 50

# reset view to fit data
renderView1.ResetCamera()

# create a new 'TTK ContourForests'
tTKContourForests1 = TTKContourForests(Input=rawFile)
...
bounds = rawFile.GetDataInformation().GetBounds()
